...
test("includes the complete risk-and-reward game loop", async () => {
  ...
  assert.match(source, /playAudio\(backgroundMusicRef\.current, 0\.28, "background music"\)/);
  assert.match(source, /if \(!nextEnabled\) stopAudio\(backgroundMusicRef\.current\)/);
  assert.match(source, /assets\/audio\/background-music\.mp3/);
  assert.match(source, /scrubbingSoundRef/);
  assert.match(source, /playAudio\(audio, 0\.42, "scrubbing sound"\)/);
  ...
  assert.match(source, /playAudio\(levelCompleteSoundRef\.current, 0\.68, "level complete sound"\)/);
  ...
  assert.match(source, /playAudio\(finalVictorySoundRef\.current, 0\.72, "final victory sound"\)/);
  ...
  assert.match(source, /playAudio\(gameOverSoundRef\.current, 0\.7, "game over sound"\)/);
  ...
  assert.match(source, /playAudio\(buttonSoundRef\.current, 0\.3, "button sound"\)/);
  ...
  assert.equal(source.match(/preload="auto"/g)?.length, 6);
  assert.match(source, /audio\.addEventListener\("canplay", retryWhenReady, \{ once: true \}\)/);
  assert.match(source, /console\.warn\(`\[audio\] Unable to play \$\{label\}`/);
  assert.match(source, /if \(statusRef\.current !== "playing"\) return;\s*statusRef\.current = nextStatus;/);
  assert.match(source, /statusRef\.current = "paused";\s*setStatus\("paused"\);/);
  assert.match(source, /const resumeGame = \(\) => \{\s*statusRef\.current = "playing";\s*setStatus\("playing"\);/);
